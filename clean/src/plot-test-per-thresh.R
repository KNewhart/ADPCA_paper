plot.test.per.thresh <- function(metric.ls, date.range=NULL, metric.n, y.lim=100) {
  ...
  if(metric=="SPE") {
    ...
    plot(as.zoo(per.of.thresh), type="p", pch=20, xlab="", ylab="Percent of SPE threshold", 
    ...
    if(any(per.of.thresh>100)) points(as.zoo(xts(rep(y.lim,length(alarm)), order.by=index(per.of.thresh)[alarm])), pch=24,bg=colors.n[1])
  }
  ...
  if(metric=="T2") {
    ...
    if(any(per.of.thresh>100)) points(as.zoo(xts(rep(y.lim,length(alarm)), order.by=index(per.of.thresh)[alarm])), pch=24,bg=colors.n[2])
  }
  ...
  if(metric=="SPE-T2") {
    ...
    if(any(per.of.thresh>100)) points(as.zoo(xts(rep(y.lim,length(alarm)), order.by=index(per.of.thresh)[alarm])), pch=24,bg=colors.n[3])
  }
