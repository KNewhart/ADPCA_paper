...
foreach.results <- list()
  ...
  graph_rawData(train.and.test.data,paste("training", rollingWindowDays[i]))
...
pdf(file="SS_SPE_T2.pdf", width=8.5, height =11)
# n <- c(1,1,2,4,3,5,6,6)
# m <- matrix(c(n), ncol=2, byrow = TRUE)
# layout(m,widths = c(0.5,0.5), heights = c(0.03,0.47,0.47,0.03))

n <- c(1,1,2,4,3,5,6,7)
m <- matrix(c(n,n+n[length(n)]), ncol=2, byrow = TRUE)
layout(m,widths = c(0.5,0.5), heights = c(0.03,0.22,0.22,0.03,0.03,0.22,0.22,0.03))
...
for(x in 1:length(foreach.results)) { # For each day
  for(y in 1:length(foreach.results[[x]])) { # For each method
    ...
    u <- par("usr")
    if(length(which(pull.t2.data[paste0(dayN)][,"SPE_Flag"] != 0)) > 0) text(0, 0.9, 
