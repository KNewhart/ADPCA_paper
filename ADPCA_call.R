...
graph_rawData(train.and.test.data,"training")
# Plot data
...
for(i in 1:length(rollingWindowDays)) {
  print(rollingWindowDays[i])
  print(nrow(train.and.test.data[paste0("/",dayOne+rollingWindowDays[i]-1)]))
  print(nrow(train.and.test.data)-nrow(train.and.test.data[paste0("/",dayOne+rollingWindowDays[i]-1)]))
  print(sum(sapply(real.faults, function(x) length(which(index(train.and.test.data) %within% x)))))
}
